...
'''
k线的回调函数onBar会有一个默认100长度的数组大小，而且在进行K线指标计算前会前判断缓存的
数目是否达标：
if not am.inited:
    #return
按照需求更改上述代码，修改缓存大小或者注释掉
'''
#%%策略回测下单问题
...
#-----2.限价单是如何进行撮合的？
'''
1.确定会成交的价格（初始最低价买入，最高价卖出）
2.开始遍历限价单字典中的所有限价单
2.1 推送委托进入队列
2.2 判断是否会成交（限价买入高于最低价，限价卖出低于最高价）
2.3 如果成交推送成交数据
2.4 撮合最优价格，确认成交下，获取开盘价和限价的最低价。
2.5 从字典删除限价单
3 结束遍历限价单字典中的所有限价单
'''
...
#-----3.停止单是如何进行撮合的？
'''
1.确定会成交的价格（初始最高价买入，最低价卖出）
2.开始遍历停止单字典中的所有停止单
2.2 判断是否会成交（停止价买入低于最高价，停止价卖出高于最低价）
2.3 如果成交推送成交数据
2.4 撮合最优价格，确认买入：获取开盘价和限价的较高价；确认卖出，获取开盘价和限价的较低价。
2.5 从字典删除停止单
3 结束遍历停止单字典中的所有停止单
'''
#%%策略回测参数优化问题
